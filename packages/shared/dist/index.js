...
var useRegressionChart = () => {
  const calculateChartDomain = react.useCallback((points, selectedFeature = 0) => {
    if (points.length === 0) {
      return {
        x: [-5, 5],
        y: [-5, 5]
  ...
  const getRegressionLinePoints = react.useCallback((type, points, stats, selectedFeature, domain) => {
    const linePoints = [];
  ...
  const generateTicks = react.useCallback((min, max, count = 5) => {
...
var useSimpleRegression = () => {
  const [simplePoints, setSimplePoints] = react.useState([]);
  const [manualMode, setManualMode] = react.useState(false);
  const [manualSlope, setManualSlope] = react.useState(0);
  const [manualIntercept, setManualIntercept] = react.useState(0);
  const calculateSimpleRegression = react.useCallback((points) => {
    if (points.length < 2) {
  ...
  const generateRandomSimplePoints = react.useCallback((count) => {
    const points = [];
...
var useMultipleRegression = () => {
  const [multiplePoints, setMultiplePoints] = react.useState([]);
  const [selectedFeature, setSelectedFeature] = react.useState(0);
  const calculateMultipleRegression = react.useCallback((points) => {
    if (points.length < 2) {
  ...
  const generateRandomMultiplePoints = react.useCallback((count) => {
    const points = [];
...
exports.useMultipleRegression = useMultipleRegression;
exports.useRegressionChart = useRegressionChart;
exports.useSimpleRegression = useSimpleRegression;
